...
from ..providers.base import VectorProvider, VectorQuery
...
class SemanticMatcher:
    ...
    async def find_resource_templates(
        self, resource_type: str, requirements: dict[str, Any], limit: int = 5
    ) -> list[SemanticMatchResult]:
        with tracer.start_as_current_span("template_matching") as span:
            span.set_attributes({"template.resource_type": resource_type, "template.limit": limit})

            try:
                requirements_str = " ".join(str(v) for v in requirements.values())
                query_text = f"template for {resource_type} with {requirements_str}"
                ...
                templates = [
                    match for match in response.matches if match.metadata.get("is_template", False)
                ]
    ...
    async def get_embedding(self, text: str) -> list[float]:
        with tracer.start_as_current_span("generate_text_embedding") as span:
            span.set_attributes({"text.length": len(text), "provider": self.vector_provider.name})
            ...
            try:
                ...
                span.set_attributes(
                    {"embedding.dimension": len(result), "embedding.generated": True}
                )
                span.set_status(Status(StatusCode.OK))
