...
class DeploymentPhaseDetector:
    ...
    @classmethod
    def detect_phase(
        cls, user_input: str, conversation_context: list[dict[str, Any]] | None = None
    ) -> DeploymentPhase:
        """
        ...
        logger.info("Detected deployment preview request (fallback)", user_input=user_input[:100])
        return DeploymentPhase.PREVIEW

    @classmethod
    def _has_recent_preview(cls, conversation_context: list[dict[str, Any]]) -> bool:
...
class DeploymentStateManager:
    ...
    def cleanup_expired_states(self) -> int:
        ...
        expired_ids = [
            deployment_id for deployment_id, state in self._states.items() if state.is_expired()
        ]
...
deployment_state_manager: DeploymentStateManager = DeploymentStateManager()
