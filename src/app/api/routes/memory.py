...
class ConversationHistoryRequest(BaseModel):
    ...
    limit: Optional[int] = Field(default=20, ge=1, le=100, description="Maximum number of messages to retrieve")
    thread_id: Optional[str] = Field(default=None, description="Filter by specific thread ID")
    include_metadata: bool = Field(default=False, description="Include message metadata in response")
    ...
    with tracer.start_as_current_span("api.memory.stats") as span:
        ...
        try:
            ...
            logger.info(
                ...
                extra={"user_id": user_email, **stats}
            )
...
async def clear_user_memory(
    current_user: Annotated[Dict[str, Any], Depends(get_optional_user)],
    thread_id: Optional[str] = Query(default=None, description="Clear specific thread only")
) -> JSONResponse:
    """
    ...
    with tracer.start_as_current_span("api.memory.clear") as span:
        ...
        try:
            ...
            if thread_id:
                ...
                raise HTTPException(
                    status_code=501,
                    detail="Thread-specific memory clearing not yet implemented"
                )
            else:
