    ...
    def get_cache_key(self, prefix: str = "") -> str:
        with tracer.start_as_current_span("schema_cache_key_generation") as span:
            ...
            cache_data = self.model_dump(
                exclude=self._cache_excluded_fields,
                exclude_none=True
            )
            content = json.dumps(cache_data, sort_keys=True, default=str)
            key_hash = hashlib.sha256(content.encode()).hexdigest()[:16]
            
            cache_key = f"{prefix}:{self.__class__.__name__}:{key_hash}" if prefix else f"{self.__class__.__name__}:{key_hash}"
            
            span.set_attributes({
                "cache.key": cache_key,
                "cache.data_size": len(content)
            })
    ...
    def cache_fingerprint(self) -> str:
        with tracer.start_as_current_span("schema_cache_fingerprint") as span:
            relevant_data = {
                ...
                if k not in self._cache_excluded_fields
            }
