...
from .base import Plugin, PluginContext, PluginResult, PluginStatus, PluginType
...
class PluginManager:
    ...
    async def initialize_all(self) -> dict[str, bool]:
        async with self._initialization_lock:
            with tracer.start_as_current_span("plugin_manager_initialize_all") as span:
                ...
                span.set_attributes(
                    {"plugins.count": len(load_order), "plugins.load_order": load_order}
                )
                ...
                for plugin_name in load_order:
                    ...
                    try:
                        ...
                        initialization_results[plugin_name] = False
                        span.add_event(
                            "plugin_initialization_failed", {"plugin": plugin_name, "error": str(e)}
                        )
                ...
                span.set_attributes(
                    {
                        ...
                        "initialization.success_rate": (
                            successful_count / len(load_order) if load_order else 0
                        ),
                    }
                )
                ...
                if successful_count == len(load_order):
                    ...
                    span.set_status(
                        Status(
                            StatusCode.ERROR,
                            (
                                f"Only {successful_count}/{len(load_order)} plugins "
    ...
    async def _initialize_plugin(self, plugin: Plugin) -> None:
        with tracer.start_as_current_span("plugin_initialization") as span:
            ...
            try:
                ...
                span.set_attributes(
                    {"plugin.initialized": True, "plugin.status": PluginStatus.ACTIVE.value}
                )
                span.set_status(Status(StatusCode.OK))
    ...
    async def execute_plugin(self, plugin_name: str, context: PluginContext) -> PluginResult:
        with tracer.start_as_current_span("plugin_execution") as span:
            span.set_attributes(
                {"plugin.name": plugin_name, "context.correlation_id": context.correlation_id}
            )
            ...
            if not plugin:
                ...
                span.set_status(Status(StatusCode.ERROR, error_msg))
                return PluginResult(
                    correlation_id=context.correlation_id, success=False, error_message=error_msg
                )
            ...
            async with execution_lock:
                try:
                    ...
                    if result.success:
                        ...
                        span.set_status(
                            Status(
                                StatusCode.ERROR, result.error_message or "Plugin execution failed"
                            )
                        )
    ...
    async def execute_plugins_by_type(
        self, plugin_type: PluginType, context: PluginContext, parallel: bool = False
        ...
        with tracer.start_as_current_span("plugins_by_type_execution") as span:
            ...
            active_plugins = [
                p for p in plugins if p.status == PluginStatus.ACTIVE and p.config.enabled
            ]

            span.set_attributes(
                {"plugins.total": len(plugins), "plugins.active": len(active_plugins)}
            )
            ...
            if parallel:
                tasks = [
                    self.execute_plugin(plugin.metadata.name, context) for plugin in active_plugins
                ]
                results = await asyncio.gather(*tasks, return_exceptions=True)

                execution_results = {}
                for plugin, result in zip(active_plugins, results, strict=False):
                    if isinstance(result, Exception):
                        ...
                        execution_results[plugin.metadata.name] = cast(PluginResult, result)
            else:
                execution_results = {}
            ...
            span.set_attributes(
                {
                    ...
                    "execution.success_rate": (
                        successful_count / len(execution_results) if execution_results else 0
                    ),
                }
            )
            ...
            if successful_count == len(execution_results):
                ...
                span.set_status(
                    Status(
                        StatusCode.ERROR,
                        f"Only {successful_count}/{len(execution_results)} executions successful",
                    )
                )
    ...
    async def reload_plugin(self, plugin_name: str) -> bool:
        with tracer.start_as_current_span("plugin_reload") as span:
            ...
            try:
                ...
                span.set_attributes({"reload.success": True, "plugin.status": plugin.status.value})
                span.set_status(Status(StatusCode.OK))
            ...
            with tracer.start_as_current_span("plugin_manager_shutdown_all") as span:
                ...
                for plugin_name in shutdown_order:
                    ...
                    try:
                        ...
                        span.add_event(
                            "plugin_shutdown_failed", {"plugin": plugin_name, "error": str(e)}
                        )
                ...
                span.set_attributes(
                    {
                        ...
                        "shutdown.success_rate": (
                            successful_count / len(shutdown_results) if shutdown_results else 0
                        ),
                    }
                )
                ...
                if successful_count == len(shutdown_results):
                    ...
                    span.set_status(
                        Status(
                            StatusCode.ERROR,
                            (
                                f"Only {successful_count}/{len(shutdown_results)} plugins "
