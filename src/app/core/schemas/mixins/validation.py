...
class ValidationMixin:
    ...
    _custom_validators: ClassVar[dict[str, Callable[[Any], Any]]] = {}
        ...
        with tracer.start_as_current_span("schema_custom_validation") as span:
            ...
            for field_name, validator_func in self._custom_validators.items():
                try:
                    ...
                    if field_value is not None and callable(validator_func):
            ...
            span.set_attributes(
                {"validation.results": validation_results, "validation.success": True}
            )
            span.set_status(Status(StatusCode.OK))
            return self

    @classmethod
    def add_validation_rule(cls, field_name: str, validator_func: Callable[[Any], Any]) -> None:
            ...
            span.set_attributes(
                {
                    ...
                    "validation.function": getattr(validator_func, "__name__", "unknown"),
                }
            )
            span.set_status(Status(StatusCode.OK))
    ...
    def validate_business_rules(self) -> list[str]:
        with tracer.start_as_current_span("schema_business_validation") as span:
            ...
            if violations:
                span.set_status(
                    Status(StatusCode.ERROR, f"{len(violations)} business rule violations")
                )
            else:
                span.set_status(Status(StatusCode.OK))
    ...
    @classmethod
    def register_business_rule(cls, rule_name: str, rule_func: Callable[[Any], bool]) -> None:
            ...
            span.set_attributes(
                {
                    ...
                    "business_rule.function": getattr(rule_func, "__name__", "unknown"),
                }
            )
            span.set_status(Status(StatusCode.OK))
