...
from app.observability.distributed_tracing import get_cross_service_tracer, get_service_tracer
...
class IntelligentAzureProvision(Tool):
    ...
    schema: dict[str, object] = {
        ...
        "properties": {
            ...
            "dry_run": {
                ...
                "default": True,
                "description": (
                    "Generate deployment preview without executing "
                    "(true for preview, false for actual deployment)"
                ),
            },
            "user_id": {
                "type": "string",
    ...
    def __init__(self, plugin_manager: PluginManager | None = None) -> None:
        ...
        self.vector_enhanced_tool: VectorEnhancedProvisioningTool | None = None
        ...
        try:
            ...
            if self.plugin_manager and not self.vector_enhanced_tool:
                self.vector_enhanced_tool = VectorEnhancedProvisioningTool(
                    self.plugin_manager, self.orchestrator
                )
        ...
        async with self.service_tracer.start_distributed_span(
            ...
            try:
                ...
                if original_request:
                    ...
                    logger.info(
                        "Proceed command detected, using stored deployment context",
                ...
                if self.vector_enhanced_tool:
                    ...
                    execution_result = (
                        await self.vector_enhanced_tool.intelligent_provision_with_context(
                    ...
                    if not parsing_result.success:
                        return cast(ToolResult, parsing_result.to_tool_result())

                    if context.dry_run:
                        preview_result = await self._generate_preview(context, parsing_result)
                        return cast(ToolResult, preview_result.to_tool_result())

                    planning_result = await self._create_deployment_plan(context)
                    if not planning_result.success:
                        return cast(ToolResult, planning_result.to_tool_result())
                ...
                return cast(ToolResult, execution_result.to_tool_result())
        ...
        with tracer.start_as_current_span("provision_validate_request") as span:
            ...
            if not request_text:
                ...
                app_insights.track_custom_event(
                    "provision_validation_failed", {"reason": "empty_request", "user_id": user_id}
                )

                return ExecutionResult.failure_result(strategy="validation", error=error_msg)
            ...
            app_insights.track_custom_event(
                ...
                {"user_id": user_id, "request_preview": request_text[:100]},
                {"request_length": len(request_text)},
            )

            return ExecutionResult.success_result(strategy="validation", data={"validated": True})
    ...
    async def _parse_request(self, context: ProvisionContext) -> ExecutionResult:
        async with self.service_tracer.start_distributed_span(
            ...
            try:
                ...
                await self.cross_service_tracer.trace_cross_service_call(
                ...
                return ExecutionResult.success_result(
                    strategy="parsing", data=context.parsed_resources
                )
                ...
                return ExecutionResult.failure_result(
                    strategy="parsing", error=f"Failed to parse request: {str(e)}"
                )

    async def _generate_preview(
        self, context: ProvisionContext, parsing_result: ExecutionResult
    ) -> ExecutionResult:
        async with self.service_tracer.start_distributed_span(
            operation_name="generate_preview",
            correlation_id=context.correlation_id,
            user_id=context.user_id,
            attributes={"preview_type": "deployment_preview", "environment": context.environment},
        ) as span:
            try:
                nlu_result = parse_provision_request(context.request_text)
                ...
                return ExecutionResult.failure_result(
                    strategy="preview_generation", error=f"Failed to generate preview: {str(e)}"
                )

    async def _handle_proceed_command(self, **kwargs: Any) -> str | None:
        """Handle proceed commands by retrieving the original deployment
            ...
            for entry in reversed(conversation_history):
                if isinstance(entry, dict) and "request" in entry:
                    ...
                    if request not in proceed_keywords and any(
                        keyword in request
                        for keyword in ["create", "deploy", "provision", "make", "add"]
                    ):
                        logger.info(
        ...
        with tracer.start_as_current_span("provision_create_deployment_plan") as span:
            ...
            try:
                if not context.parsed_resources:
                    ...
                    return ExecutionResult.failure_result(strategy="planning", error=error_msg)

                deployment_plan = {
                    "resources": context.parsed_resources,
                    "deployment_order": [
                        r.get("resource_name", f"resource_{i}")
                        for i, r in enumerate(context.parsed_resources)
                    ],
                    "parallel_groups": [],
                ...
                span.set_attributes(
                    {
                        ...
                        "planning.estimated_time_minutes": float(
                            str(deployment_plan.get("estimated_time_minutes", 0))
                        ),
                ...
                return ExecutionResult.success_result(strategy="planning", data=deployment_plan)
                ...
                return ExecutionResult.failure_result(
                    strategy="planning", error=f"Failed to create deployment plan: {str(e)}"
                )
    ...
    async def _store_user_context(self, context: ProvisionContext) -> None:
        async with self.service_tracer.start_distributed_span(
            ...
            correlation_id=context.correlation_id,
            user_id=context.user_id,
            attributes={"storage_type": "agent_memory", "persistence_layer": "postgresql"},
        ) as span:
            try:
                await self.cross_service_tracer.trace_cross_service_call(
                    from_service="intelligent_provision_service",
                ...
                await memory.store_context(
                    ...
                    agent_name="intelligent_provision",
                    context_key="conversation_history",
                    context_data={"conversations": existing_conversation},
                    correlation_id=context.correlation_id,
                    ttl=timedelta(days=7),
                )
    ...
    async def _store_execution_result(
        self, context: ProvisionContext, result: ExecutionResult
    ) -> None:
