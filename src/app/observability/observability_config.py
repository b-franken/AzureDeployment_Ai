...
class ObservabilityConfig:
    ...
    def initialize_telemetry(self) -> None:
        ...
        try:
            resource = Resource.create(
                {
                    ...
                    "deployment.type": (
                        "containerized" if os.environ.get("CONTAINER_MODE") else "standalone"
                    ),
                }
            )
            ...
            if getattr(settings.observability, "enable_otlp_export", False):
                otlp_exporter = OTLPSpanExporter(
                    endpoint=getattr(settings.observability, "otel_exporter_otlp_endpoint", None),
                    headers={
                        "authorization": (
                            "Bearer "
                            + getattr(settings.observability, "otel_exporter_otlp_headers", "")
                        )
                    },
                )
            ...
            logger.info(
                ...
                service_version=self.service_version,
                environment=self.environment,
                otlp_enabled=getattr(settings.observability, "enable_otlp_export", False),
            )
            ...
            app_insights.track_custom_event(
                "observability_initialized",
                {
                    ...
                    "environment": self.environment,
                    "otlp_enabled": str(
                        getattr(settings.observability, "enable_otlp_export", False)
                    ),
                },
            )
            ...
            logger.error(
                "Failed to initialize observability telemetry", error=str(e), exc_info=True
            )
    ...
    def _instrument_libraries(self) -> None:
        try:
            ...
            logger.warning("Failed to instrument some libraries for tracing", error=str(e))
    ...
    def get_tracing_health(self) -> dict[str, Any]:
        ...
        return {
            ...
            "otlp_export_enabled": getattr(settings.observability, "enable_otlp_export", False),
        }
