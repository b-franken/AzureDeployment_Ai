...
from .execution_context import ExecutionResult, ProvisionContext, ProvisioningPhase
...
class ProvisioningOrchestrator:
        ...
        with tracer.start_as_current_span("orchestrator_initialize_strategies") as span:
            ...
            if successful_count == len(self._strategies):
                ...
                span.set_status(
                    Status(
                        StatusCode.ERROR,
                        f"Only {successful_count}/{len(self._strategies)} strategies initialized",
                    )
                )
    ...
    async def execute_with_fallback(self, context: ProvisionContext) -> ExecutionResult:
        async with self.service_tracer.start_distributed_span(
            ...
            for strategy in self._strategies:
                ...
                try:
                    ...
                    with tracer.start_as_current_span("strategy_execution") as strategy_span:
                        ...
                        execution_time = (
                            datetime.now(UTC) - strategy_start_time
                        ).total_seconds() * 1000
                        ...
                        if result.success:
                            ...
                            strategy_span.set_status(
                                Status(StatusCode.ERROR, result.error_message or "Strategy failed")
                            )
            ...
            total_execution_time = sum(
                float(attempt["execution_time_ms"])
                for attempt in execution_attempts
                if isinstance(attempt["execution_time_ms"], int | float)
            )
    ...
    def _update_strategy_stats(
        self, strategy_name: str, success: bool, execution_time_ms: float
    ) -> None:
        ...
        stats["avg_execution_time_ms"] = (
            (current_avg * (total_attempts - 1)) + execution_time_ms
        ) / total_attempts
        ...
        with tracer.start_as_current_span("orchestrator_cleanup_strategies") as span:
            ...
            for strategy in self._strategies:
                try:
                    ...
                    span.add_event(
                        "strategy_cleanup_failed", {"strategy": strategy.name, "error": str(e)}
                    )
