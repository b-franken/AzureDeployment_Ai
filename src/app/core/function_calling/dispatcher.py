...
import asyncio
import inspect
from typing import Any, Callable, Dict, TypeVar, Generic, get_type_hints
...
class FunctionDispatcher:
            ...
            try:
                ...
                if not func_info:
                    ...
                    return self._create_error_response(
                        request, error_msg, start_time
                    )
                
                span.set_attributes({
                    "function.type": func_info.function_type,
                    "function.input_schema": func_info.input_schema.__name__ if func_info.input_schema else "None",
                    "function.output_schema": func_info.output_schema.__name__ if func_info.output_schema else "None"
                })
                
                validated_args = await self._validate_input(
                    request.arguments, 
                    func_info.input_schema,
                    span
                )
    ...
    async def _validate_input(self, 
                             ...
                             span: trace.Span) -> Dict[str, Any]:
        if input_schema is None:
            return arguments
        
        with tracer.start_as_current_span("input_validation", context=trace.set_span_in_context(span)):
            try:
    ...
    async def _validate_output(self,
        ...
        with tracer.start_as_current_span("output_validation", context=trace.set_span_in_context(span)):
            try:
    ...
    async def _execute_function(self,
                               ...
                               timeout: int,
                               span: trace.Span) -> Any:
        with tracer.start_as_current_span("function_execution", context=trace.set_span_in_context(span)):
            if inspect.iscoroutinefunction(handler):
                task = asyncio.create_task(handler(**arguments, **context))
                self._active_calls[span.get_span_context().span_id] = task
                
                try:
                    result = await asyncio.wait_for(task, timeout=timeout)
                    return result
                finally:
                    self._active_calls.pop(span.get_span_context().span_id, None)
            else:
    ...
    def _create_error_response(self,
                              ...
                              error_message: str,
                              start_time: datetime) -> FunctionCallResponse:
        execution_time = (datetime.now(UTC) - start_time).total_seconds() * 1000
    ...
    async def cancel_execution(self, correlation_id: str) -> bool:
        with tracer.start_as_current_span("function_cancellation") as span:
            ...
            span.set_attributes({
                "cancellation.count": cancelled_count,
                "cancellation.success": cancelled_count > 0
            })
            span.set_status(Status(StatusCode.OK))
