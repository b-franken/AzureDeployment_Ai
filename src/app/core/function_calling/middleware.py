...
class ValidationMiddleware:
    ...
    def add_context_validator(self, validator: Callable[[Dict[str, Any]], ValidationResult]) -> None:
    ...
    async def validate_input(self, arguments: Dict[str, Any], function_name: str) -> ValidationResult:
        with tracer.start_as_current_span("middleware_input_validation") as span:
            ...
            for validator in self._pre_validators:
                try:
                    ...
                    if not result.is_valid:
                        span.add_event("pre_validation_failed", {
            ...
            if not is_valid:
                span.set_status(Status(StatusCode.ERROR, f"Input validation failed with {len(all_errors)} errors"))
            else:
                span.set_status(Status(StatusCode.OK))
    ...
    async def validate_output(self, result: Any, function_name: str) -> ValidationResult:
        with tracer.start_as_current_span("middleware_output_validation") as span:
            ...
            for validator in self._post_validators:
                try:
                    ...
                    if not validation_result.is_valid:
                        span.add_event("post_validation_failed", {
                            "validator": validator.__name__,
            ...
            if not is_valid:
                span.set_status(Status(StatusCode.ERROR, f"Output validation failed with {len(all_errors)} errors"))
            else:
                span.set_status(Status(StatusCode.OK))
            
            return validation_result
    
    async def validate_context(self, context: Dict[str, Any], function_name: str) -> ValidationResult:
        with tracer.start_as_current_span("middleware_context_validation") as span:
            ...
            for validator in self._context_validators:
                try:
                    ...
                    if not validation_result.is_valid:
                        span.add_event("context_validation_failed", {
                            "validator": validator.__name__,
                            "errors": validation_result.errors
                        })
                except Exception as e:
            ...
            if not is_valid:
                span.set_status(Status(StatusCode.ERROR, f"Context validation failed with {len(all_errors)} errors"))
            else:
                span.set_status(Status(StatusCode.OK))
