...
class IntelligentErrorHandler:
    ...
    async def analyze_error(
        ...
        with tracer.start_as_current_span(
            ...
            root_cause = await self._determine_root_cause(error_message, context, deployment_history)
        ...
        if "cores" in error_message or "vm" in resource_type.lower():
            ...
            if vm_sku and vm_sku in self._sku_alternatives.get("virtual_machines", {}):
                ...
                analysis.alternative_configurations = [
                    {"vm_size": alt, "reason": f"Alternative to {vm_sku}"} for alt in alternatives[:3]
                ]
        
        analysis.suggested_actions = [
            f"Request quota increase for {resource_type} in {current_location}",
            f"Retry deployment in alternative region: {analysis.recommended_regions[0] if analysis.recommended_regions else 'northeurope'}",
    ...
    async def _analyze_configuration_error(
        ...
        analysis.suggested_actions = [
            f"Switch to available SKU: {analysis.alternative_configurations[0]['sku'] if analysis.alternative_configurations else 'Standard_B1s'}",
    ...
    async def _analyze_network_error(
        ...
        if current_cidr:
            ...
            available_cidrs = [cidr for cidr in self._network_cidr_pool if cidr not in used_cidrs][:3]
        else:
        ...
        analysis.alternative_configurations = [
            {
                ...
                "reason": f"Alternative network range to avoid conflicts",
            }
            for cidr in available_cidrs
        ]
        
        analysis.suggested_actions = [
            f"Use alternative CIDR range: {available_cidrs[0] if available_cidrs else '10.1.0.0/16'}",
    ...
    async def _analyze_security_error(
        ...
        analysis.suggested_actions = [
            f"Grant '{required_roles[0]}' role to {principal_id}" if required_roles else "Review service principal permissions",
    ...
    async def _analyze_dependency_error(
        ...
        analysis.suggested_actions = [
            f"Create missing dependency: {missing_dependency}" if missing_dependency else "Verify all dependencies exist",
    ...
    async def generate_remediation_plan(
        ...
        with tracer.start_as_current_span(
            ...
            primary_action = analysis.suggested_actions[0] if analysis.suggested_actions else "Review configuration"
            backup_actions = analysis.suggested_actions[1:] if len(analysis.suggested_actions) > 1 else []
    ...
    def _calculate_success_probability(self, analysis: ErrorAnalysis, context: dict[str, Any]) -> float:
    ...
    def _determine_execution_order(self, analysis: ErrorAnalysis, context: dict[str, Any]) -> list[str]:
