...
class SchemaRegistry:
    ...
    def register_schema(
        self, schema_cls: type[T], version: str = "1.0.0", category: str = "general"
    ) -> type[T]:
        with tracer.start_as_current_span("schema_registration") as span:
            ...
            if schema_name in self._schemas:
                ...
                span.add_event(
                    ...
                    {"existing_version": existing_version, "new_version": version},
                )
    ...
    def _analyze_dependencies(self, schema_cls: type[BaseModel]) -> None:
        ...
        for _field_name, field_info in schema_cls.model_fields.items():
    ...
    def list_schemas(self, category: str | None = None) -> dict[str, type[BaseModel]]:
        with tracer.start_as_current_span("schema_listing") as span:
            span.set_attributes(
                {"filter.category": category or "all", "total_schemas": len(self._schemas)}
            )
    ...
    def get_dependencies(self, schema_name: str) -> set[str]:
        with tracer.start_as_current_span("schema_dependencies") as span:
            ...
            span.set_attributes({"dependencies.count": len(deps), "dependencies.list": list(deps)})
            span.set_status(Status(StatusCode.OK))
            return deps
            ...
            span.set_attributes(
                {
                    ...
                    "integrity.ok": 1 if integrity_report["integrity_ok"] else 0,
                }
            )
