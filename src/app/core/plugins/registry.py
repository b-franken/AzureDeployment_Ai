...
from .base import Plugin, PluginConfig, PluginMetadata, PluginStatus, PluginType
...
class PluginRegistry:
        ...
        with tracer.start_as_current_span("plugin_registration") as span:
            ...
            span.set_attributes(
                {
                    ...
                    "plugin.dependencies": list(plugin.metadata.dependencies or []),
                }
            )
            ...
            if plugin_name in self._plugins:
                ...
                span.add_event(
                    ...
                    {"existing_version": existing_version, "new_version": plugin.metadata.version},
                )

            try:
                dependency_errors = self._validate_dependencies(plugin.metadata.dependencies or [])
                ...
                self._plugin_dependencies[plugin_name] = plugin.metadata.dependencies or []
                ...
                span.set_attributes(
                    {
                        ...
                        "registry.plugins_of_type": len(
                            self._plugin_types[plugin.metadata.plugin_type]
                        ),
                    }
                )
    ...
    def get_plugin(self, name: str) -> Plugin | None:
        with tracer.start_as_current_span("plugin_retrieval") as span:
            span.set_attributes(
                {"plugin.name": name, "available_plugins": list(self._plugins.keys())}
            )
            ...
            span.set_attributes(
                {
                    ...
                    "available_types": [
                        pt.value
                        for pt in set(p.metadata.plugin_type for p in self._plugins.values())
                    ],
                    "available_statuses": [
                        ps.value for ps in set(p.status for p in self._plugins.values())
                    ],
                }
            )
            span.set_status(Status(StatusCode.OK))
            ...
            span.set_attributes(
                {
                    ...
                    "result.active_count": len(
                        [p for p in plugins if p.status == PluginStatus.ACTIVE]
                    ),
                }
            )
            span.set_status(Status(StatusCode.OK))
    ...
    def get_load_order(self) -> list[str]:
        with tracer.start_as_current_span("plugin_load_order") as span:
            span.set_attributes(
                {"load_order.count": len(self._load_order), "load_order.plugins": self._load_order}
            )
            span.set_status(Status(StatusCode.OK))
    ...
    def unregister_plugin(self, name: str) -> bool:
        with tracer.start_as_current_span("plugin_unregistration") as span:
            ...
            try:
                ...
                if dependents:
                    error_msg = (
                        f"Cannot unregister plugin {name}: still has dependents {dependents}"
                    )
                    span.set_status(Status(StatusCode.ERROR, error_msg))
                    return False
    ...
    def _update_load_order(self) -> None:
        with tracer.start_as_current_span("update_load_order") as span:
            ...
            try:
                plugin_priority = {
                    name: plugin.config.load_priority for name, plugin in self._plugins.items()
                }
                ...
                span.set_attributes(
                    {"load_order.success": True, "load_order.count": len(load_order)}
                )
                span.set_status(Status(StatusCode.OK))
    ...
    def get_registry_stats(self) -> dict[str, Any]:
        with tracer.start_as_current_span("plugin_registry_stats") as span:
            ...
            plugins_by_status = stats["plugins_by_status"]
            has_circular_deps = stats["has_circular_dependencies"]

            active_count = 0
            if isinstance(plugins_by_status, dict):
                active_count = plugins_by_status.get(PluginStatus.ACTIVE.value, 0)

            span.set_attributes(
                {
                    "stats.total_plugins": (
                        int(total_plugins) if isinstance(total_plugins, int) else 0
                    ),
                    "stats.active_plugins": (
                        int(active_count) if isinstance(active_count, int) else 0
                    ),
                    "stats.has_circular_deps": (
                        bool(has_circular_deps) if isinstance(has_circular_deps, bool) else False
                    ),
                }
            )
            span.set_status(Status(StatusCode.OK))
        ...
        with tracer.start_as_current_span("plugin_registry_health_check") as span:
            ...
            if healthy_count == len(self._plugins):
                ...
                span.set_status(
                    Status(
                        StatusCode.ERROR,
                        f"Only {healthy_count}/{len(self._plugins)} plugins healthy",
                    )
                )
...
def register_plugin(metadata: PluginMetadata, config: PluginConfig | None = None) -> Any:
