...
class FunctionRegistry:
        ...
        with tracer.start_as_current_span("function_registration") as span:
            ...
            if name in self._functions:
                span.add_event(
                    ...
                    {"existing_type": self._functions[name].function_type.value},
                )
    ...
    def unregister_function(self, name: str) -> bool:
        with tracer.start_as_current_span("function_unregistration") as span:
            ...
            if name in self._functions:
                ...
                span.set_attributes(
                    {"function.unregistered": True, "function.type": func_info.function_type.value}
                )
                span.set_status(Status(StatusCode.OK))
                return True
            else:
    ...
    def get_function_stats(self) -> dict[str, Any]:
        with tracer.start_as_current_span("function_registry_stats") as span:
            ...
            with_input_schema = stats["functions_with_input_schema"]
            with_output_schema = stats["functions_with_output_schema"]

            span.set_attributes(
                {
                    "stats.total_functions": (
                        int(total_functions) if isinstance(total_functions, int) else 0
                    ),
                    "stats.with_input_schema": (
                        int(with_input_schema) if isinstance(with_input_schema, int) else 0
                    ),
                    "stats.with_output_schema": (
                        int(with_output_schema) if isinstance(with_output_schema, int) else 0
                    ),
                }
            )
            span.set_status(Status(StatusCode.OK))
