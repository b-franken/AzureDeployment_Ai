...
class AVMStrategy(ProvisioningStrategy):
        ...
        with tracer.start_as_current_span("avm_strategy_initialize") as span:
            try:
                ...
                app_insights.track_custom_event(
                    "avm_strategy_initialized",
                    {"backend_type": "BicepAvmBackend"}
                )
                ...
                span.record_exception(e)
                span.set_status(Status(StatusCode.ERROR, f"Failed to initialize AVM backend: {str(e)}"))
                raise
    ...
    async def execute(self, context: ProvisionContext) -> ExecutionResult:
        ...
        with tracer.start_as_current_span("avm_strategy_execute") as span:
            ...
            try:
                from app.tools.provision.backends.avm_bicep.engine import ProvisionContext as AVMContext
                ...
                avm_context = AVMContext(
                    ...
                    resource_group=nlu_result.parameters.get("resource_group") or context.resource_group or "default-rg",
                    location=nlu_result.parameters.get("location") or context.location or "westeurope",
                ...
                if context.dry_run:
                    result = await self._avm_backend.plan_from_nlu(nlu_result, avm_context, context.dry_run)
                    operation_type = "plan"
                else:
                    plan_result = await self._avm_backend.plan_from_nlu(nlu_result, avm_context, False)
                    if hasattr(plan_result, 'bicep_path') and plan_result.bicep_path:
                        result = await self._avm_backend.apply(avm_context, plan_result.bicep_path)
                    else:
                ...
                if success:
                    ...
                    if isinstance(result, dict):
                        if 'resources' in result and result['resources']:
                            resources_affected = [r.get('name', 'unnamed') for r in result['resources']]
                        if 'warnings' in result and result['warnings']:
                            warnings = result['warnings']
                        deployment_id = result.get('deployment_id', '')
                        if deployment_id:
                            resource_name_from_deployment = deployment_id.split('/')[-1].replace('avm-deployment-', '')
...
class SDKFallbackStrategy(ProvisioningStrategy):
    ...
    async def initialize(self) -> None:
        with tracer.start_as_current_span("sdk_fallback_initialize") as span:
            try:
                ...
                app_insights.track_custom_event(
                    "sdk_fallback_strategy_initialized",
                    {"backend_type": "AzureActionRegistry"}
                )
                
            except ImportError as e:
                span.record_exception(e)
                span.set_status(Status(StatusCode.ERROR, f"Failed to import SDK fallback: {str(e)}"))
                raise
            except Exception as e:
                span.record_exception(e)
                span.set_status(Status(StatusCode.ERROR, f"Failed to initialize SDK fallback: {str(e)}"))
                raise
    ...
    async def can_handle(self, context: ProvisionContext) -> bool:
        with tracer.start_as_current_span("sdk_fallback_can_handle") as span:
            ...
            can_handle = (
                self._intelligent_tool is not None and
                self.name not in context.attempted_strategies and
                "avm_bicep" in context.attempted_strategies
            )
    ...
    async def execute(self, context: ProvisionContext) -> ExecutionResult:
        ...
        with tracer.start_as_current_span("sdk_fallback_execute") as span:
            ...
            try:
                ...
                # action_result is a tuple: (status, result)
                status, result = action_result
                success = status in ["created", "exists", "plan"]
                
                span.set_attributes({
                    "sdk_fallback.execution_time_ms": execution_time,
                    "sdk_fallback.success": success,
                    "sdk_fallback.status": status
                })
                
                if success:
                    resources_affected = self._extract_resource_names(nlu_result, status, result)
                    warnings = []
                    ...
                    error_message = f"SDK fallback failed with status: {status}"
    ...
    async def _execute_resource_action(self, nlu_result, context: ProvisionContext, dry_run: bool):
        ...
        if not action_name:
            return ("failed", f"No action mapping found for resource type: {resource_type}")
        
        action_fn = self._action_registry(action_name)
        if not action_fn:
            return ("failed", f"Action function not found: {action_name}")
    ...
    def _build_action_parameters(self, nlu_result, context: ProvisionContext, action_name: str) -> Dict[str, Any]:
        resource_type = nlu_result.resource_type or ""
    ...
    def _build_storage_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "storage_name": params.get("name") or f"{context.name_prefix}storage{context.environment}",
            "location": params.get("location") or context.location or "westeurope",
            "sku": params.get("sku") or ("Standard_ZRS" if context.environment == "prod" else "Standard_LRS"),
            "tags": {**context.tags, "environment": context.environment}
        }
    
    def _build_webapp_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
    ...
    def _build_aks_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "cluster_name": params.get("name") or f"{context.name_prefix}aks{context.environment}",
            "location": params.get("location") or context.location or "westeurope",
            "dns_prefix": params.get("dns_prefix") or f"{context.name_prefix}aks{context.environment}",
    ...
    def _build_acr_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            ...
            "admin_user_enabled": params.get("admin_user_enabled") or (context.environment != "prod"),
            "tags": {**context.tags, "environment": context.environment}
        }
    ...
    def _build_sql_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "server_name": params.get("server_name") or f"{context.name_prefix}sql{context.environment}",
            "location": params.get("location") or context.location or "westeurope",
            "administrator_login": params.get("sql_admin_user") or "sqladmin",
            "administrator_password": params.get("sql_admin_password") or self._generate_secure_password(),
            "version": params.get("version") or "12.0",
            "tags": {**context.tags, "environment": context.environment}
        }
    
    def _build_keyvault_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "vault_name": params.get("vault_name") or params.get("name") or f"{context.name_prefix}kv{context.environment}",
            "location": params.get("location") or context.location or "westeurope",
    ...
    def _build_cosmos_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "account_name": params.get("account_name") or params.get("name") or f"{context.name_prefix}cosmos{context.environment}",
            "location": params.get("location") or context.location or "westeurope",
    ...
    def _build_redis_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
    ...
    def _build_vnet_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
    ...
    def _build_subnet_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "vnet_name": params.get("vnet_name") or f"{context.name_prefix}vnet{context.environment}",
            "subnet_name": params.get("name") or "default",
            "address_prefix": params.get("address_prefix") or "10.0.1.0/24",
            "tags": {**context.tags, "environment": context.environment}
        }
    
    def _build_nsg_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
    ...
    def _build_public_ip_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
    ...
    def _build_app_gateway_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "gateway_name": params.get("name") or f"{context.name_prefix}appgw{context.environment}",
            "location": params.get("location") or context.location or "westeurope",
            "sku_name": params.get("sku") or ("WAF_v2" if context.environment == "prod" else "Standard_v2"),
    ...
    def _build_plan_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
    ...
    def _build_log_analytics_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "workspace_name": params.get("name") or f"{context.name_prefix}law{context.environment}",
            "location": params.get("location") or context.location or "westeurope",
            "sku": params.get("sku") or "PerGB2018",
            "retention_in_days": params.get("retention") or (90 if context.environment == "prod" else 30),
            "tags": {**context.tags, "environment": context.environment}
        }
    
    def _build_app_insights_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "app_insights_name": params.get("name") or f"{context.name_prefix}ai{context.environment}",
            "location": params.get("location") or context.location or "westeurope",
    ...
    def _build_managed_identity_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
    ...
    def _build_private_dns_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
    ...
    def _build_private_endpoint_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
    ...
    def _build_apim_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            ...
            "sku_name": params.get("sku") or ("Premium" if context.environment == "prod" else "Developer"),
            "tags": {**context.tags, "environment": context.environment}
        }
    
    def _build_eventhub_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "namespace_name": params.get("namespace_name") or f"{context.name_prefix}eh{context.environment}",
    ...
    def _build_default_params(self, params: Dict[str, Any], context: ProvisionContext) -> Dict[str, Any]:
        return {
            "subscription_id": context.subscription_id,
            "resource_group": params.get("resource_group") or context.resource_group or "default-rg",
            "location": params.get("location") or context.location or "westeurope",
    ...
    def _extract_resource_names(self, nlu_result, status: str, result: Any) -> List[str]:
        resource_names = []
