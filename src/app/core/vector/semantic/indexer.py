...
class ResourceIndexRequest(BaseSchema):
    ...
    content_override: str | None = Field(default=None, description="Custom content for indexing")
    embedding_override: list[float] | None = Field(
        default=None, description="Pre-computed embedding"
    )
...
class ResourceIndexResult(BaseSchema):
    ...
    error_message: str | None = Field(default=None, description="Error if indexing failed")
            ...
            try:
                ...
                if self.collection_name not in collections:
                    ...
                    if success:
                        ...
                        self.logger.error(
                            "collection_creation_failed", collection=self.collection_name
                        )
                        return False
                else:
                    span.set_status(Status(StatusCode.OK))
        ...
        with tracer.start_as_current_span("index_single_resource") as span:
            ...
            try:
                ...
                existing_vector = await self.vector_provider.get_vector_by_id(
                    collection_name=self.collection_name, vector_id=resource.resource_id
                )
                ...
                success = await self.vector_provider.upsert_vectors(
                    collection_name=self.collection_name, vectors=vector_data
                )
        ...
        with tracer.start_as_current_span("delete_resource_index") as span:
            ...
            try:
                success = await self.vector_provider.delete_vectors(
                    collection_name=self.collection_name, vector_ids=[resource_id]
                )

                span.set_attributes(
                    {"deletion.success": success, "collection.name": self.collection_name}
                )
