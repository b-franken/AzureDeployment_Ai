...
from app.core.schemas.mixins import AzureMixin, CacheMixin
...
class ProvisionContext(TimestampedSchema, AzureMixin, CacheMixin):
    ...
    parsed_resources: List[Dict[str, Any]] = Field(default_factory=list, description="Parsed resource requirements")
    deployment_plan: Dict[str, Any] = Field(default_factory=dict, description="Generated deployment plan")
    execution_metadata: Dict[str, Any] = Field(default_factory=dict, description="Execution metadata")
    
    current_phase: ProvisioningPhase = Field(default=ProvisioningPhase.VALIDATION, description="Current execution phase")
    attempted_strategies: List[str] = Field(default_factory=list, description="List of attempted provisioning strategies")
    ...
    conversation_context: List[Dict[str, str]] = Field(default_factory=list, description="Conversation history")
    ...
    def advance_phase(self, new_phase: ProvisioningPhase | str) -> None:
        with tracer.start_as_current_span("context_advance_phase") as span:
            ...
            old_phase_value = old_phase.value if isinstance(old_phase, ProvisioningPhase) else str(old_phase)
            new_phase_value = new_phase.value if isinstance(new_phase, ProvisioningPhase) else str(new_phase)
...
class ExecutionResult(BaseSchema):
    ...
    resources_affected: List[str] = Field(default_factory=list, description="Resources that were created/modified")
    
    observability_data: Dict[str, Any] = Field(default_factory=dict, description="Telemetry and metrics data")
    ...
    def success_result(cls, 
                      ...
                      output: Any = None) -> "ExecutionResult":
    ...
    def failure_result(cls,
                      ...
                      error: str,
                      execution_time: float = 0.0,
                      warnings: List[str] = None) -> "ExecutionResult":
    ...
    def to_tool_result(self) -> Dict[str, Any]:
        if self.success and self.strategy_used == "preview_generation" and isinstance(self.result_data, dict):
        ...
        return {
            "ok": self.success,
            "summary": f"Strategy '{self.strategy_used}' {'succeeded' if self.success else 'failed'}",
