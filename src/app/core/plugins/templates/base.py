...
class TemplateEngine(ABC):
    ...
    async def render_template(self, 
                             ...
                             context: TemplateContext) -> TemplateRenderResult:
        pass
        ...
        with tracer.start_as_current_span("template_registration") as span:
            ...
            try:
                ...
                span.set_attributes({
                    ...
                    "registry.total_templates": len(self._templates)
                })
    ...
    def get_template(self, name: str) -> ResourceTemplate | None:
        with tracer.start_as_current_span("template_retrieval") as span:
            ...
            span.set_attributes({
                ...
                "template.category": template.metadata.category.value if template else "unknown"
            })
    ...
    def list_templates(self, 
                      ...
                      format: TemplateFormat | None = None) -> Dict[str, ResourceTemplate]:
        with tracer.start_as_current_span("template_listing") as span:
            ...
            span.set_attributes({
                ...
                "available_categories": [tc.value for tc in set(t.metadata.category for t in self._templates.values())],
                "available_formats": [tf.value for tf in set(t.metadata.format for t in self._templates.values())]
            })
            span.set_status(Status(StatusCode.OK))
            
            return filtered_templates
    
    async def render_with_inheritance(self, 
                                    template: ResourceTemplate,
                                    context: TemplateContext) -> TemplateRenderResult:
            ...
            try:
                if template.parent_template:
                    ...
                    if not parent_result.success:
                        span.set_status(Status(StatusCode.ERROR, "Parent template rendering failed"))
                        return parent_result
                    
                    merged_content = self._merge_templates(parent_result.rendered_template, template.template_content)
                ...
                span.set_attributes({
                    ...
                    "render.warnings": len(result.warnings)
                })
                
                if result.success:
                    span.set_status(Status(StatusCode.OK))
                else:
                    span.set_status(Status(StatusCode.ERROR, result.error_message or "Template rendering failed"))
    ...
    def validate_parameters(self, 
                          template: ResourceTemplate, 
                          parameter_values: Dict[str, Any]) -> List[str]:
        with tracer.start_as_current_span("template_parameter_validation") as span:
            ...
            for param in template.parameters:
                ...
                if param.allowed_values and value not in param.allowed_values:
                    errors.append(f"Parameter '{param.name}' value '{value}' not in allowed values: {param.allowed_values}")
                
                if param.validation_pattern:
                    import re
                    if not re.match(param.validation_pattern, str(value)):
                        errors.append(f"Parameter '{param.name}' value '{value}' does not match pattern: {param.validation_pattern}")
            ...
            if errors:
                span.set_status(Status(StatusCode.ERROR, f"Parameter validation failed with {len(errors)} errors"))
            else:
                span.set_status(Status(StatusCode.OK))
