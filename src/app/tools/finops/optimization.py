...
class OptimizationService:
    ...
    async def analyze_optimization_opportunities(
        ...
        tasks.extend(
            [
                self._analyze_aks_optimization(
                    subscription_id, r, costs.get(r["id"], {}), strategy)
                for r in resources
    ...
    async def _analyze_vm_optimization(
        ...
        if cpu_metrics:
            avg_cpu = sum(m["average"]
                          for m in cpu_metrics if m["average"]) / len(cpu_metrics)
            ...
            if avg_cpu < 20:
                recommendations.append(
                    OptimizationRecommendation(
                        ...
                        description=f"VM {resource['name']} has very low usage ({avg_cpu:.1f}%). Consider auto-shutdown or deletion.",
                        estimated_monthly_savings=monthly_cost * 0.8,
                        estimated_savings_percentage=80.0,
                        implementation_effort="low",
                        risk_level="medium",
                        actions=[
                            {"action": "verify_vm_necessity",
    ...
    async def _analyze_storage_optimization(
        ...
        if sku.get("name", "").startswith("Standard_GRS") and strategy != OptimizationStrategy.CONSERVATIVE:
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    description=f"Storage account {resource['name']} uses GRS. Consider LRS for non-critical data.",
                    estimated_monthly_savings=monthly_cost * 0.5,
                    estimated_savings_percentage=50.0,
                    implementation_effort="low",
                    risk_level="medium" if strategy == OptimizationStrategy.AGGRESSIVE else "high",
                    actions=[
    ...
    async def _analyze_aks_optimization(
        ...
        for pool in agent_pools:
            if not pool.get("enableAutoScaling", False):
                recommendations.append(
                    OptimizationRecommendation(
                        id=f"opt-aks-{resource['name']}-autoscale-{pool.get('name', 'default')}",
                        resource_id=resource_id,
                        resource_type="AKS Cluster",
                        recommendation_type="enable_autoscaling",
                        description=f"Enable autoscaling for node pool {pool.get('name', 'default')}",
        ...
        if strategy == OptimizationStrategy.AGGRESSIVE:
            spot_pool_rec = self._suggest_spot_node_pool(
    ...
    async def _analyze_database_optimization(
                ...
                if avg_cpu < 20:
                    recommendations.append(
                        OptimizationRecommendation(
                            ...
                            description=f"SQL Database has low usage. Consider elastic pool for multiple databases.",
        ...
        if strategy != OptimizationStrategy.CONSERVATIVE:
            serverless_rec = self._suggest_serverless_compute(
                resource, monthly_cost)
            if serverless_rec:
            ...
            if resource["location"] == rec.get("location"):
                ...
                if savings > 0:
                    return OptimizationRecommendation(
                        ...
                        description=f"Purchase {rec.get('recommended_quantity', 1)} year reservation for consistent savings",
                        estimated_monthly_savings=savings / 12,
                        estimated_savings_percentage=35.0,
                        implementation_effort="low",
                        risk_level="low",
                        actions=[
                            {"action": "review_usage_history",
                                "period": "last 3 months"},
                            {"action": "confirm_long_term_need",
                                "duration": "1-3 years"},
                            {"action": "calculate_break_even",
                                "metric": "months to ROI"},
    ...
    async def _check_spot_opportunity(
        ...
        return OptimizationRecommendation(
            ...
            risk_level="high",
            actions=[
                {"action": "assess_interruption_tolerance",
                    "requirement": "stateless workload"},
                {"action": "implement_graceful_shutdown",
                    "handler": "deallocate notification"},
                {"action": "setup_restart_automation",
                    "tool": "Azure Automation or Logic Apps"},
    ...
    async def _analyze_storage_lifecycle(
        ...
        if properties.get("isHnsEnabled", False):
            return OptimizationRecommendation(
                ...
                description=f"Implement lifecycle policies to move cold data to cheaper tiers",
    ...
    def _suggest_spot_node_pool(
        ...
        return OptimizationRecommendation(
            ...
            risk_level="medium",
            actions=[
                {"action": "identify_tolerant_workloads",
                    "examples": "batch jobs, testing"},
    ...
    def _suggest_serverless_compute(
        ...
        return OptimizationRecommendation(
            ...
            actions=[
                ...
                {"action": "configure_auto_pause",
                    "delay": "1 hour of inactivity"},
        ...
        )f"opt-vm-{resource['name']}-rightsize",
                        resource_id = resource_id,
                        resource_type = "Virtual Machine",
                        recommendation_type = "right_sizing",
                        description = f"VM {resource['name']} has average CPU usage of {avg_cpu:.1f}%. Consider downsizing.",
                        estimated_monthly_savings = monthly_cost * 0.3,
                        estimated_savings_percentage = 30.0,
                        implementation_effort = "medium",
                        risk_level = "low" if strategy == OptimizationStrategy.CONSERVATIVE else "medium",
                        actions = [
                            {"action": "analyze_workload_patterns",
                                "duration": "1 week"},
                            {"action": "identify_smaller_sku",
                                "target": "reduce by 1-2 sizes"},
                            {"action": "schedule_maintenance_window",
                                "downtime": "5-10 minutes"},
                            {"action": "resize_vm",
                                "automation": "Azure CLI or Portal"},
                            {"action": "monitor_performance",
                                "duration": "1 week post-change"},
                        ],
                        validation_steps = [
