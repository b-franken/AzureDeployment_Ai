...
import json
import re
from typing import Any, Dict, Type, TypeVar, Generic, Union
...
class StructuredOutputParser:
        ...
        with tracer.start_as_current_span("structured_output_parsing") as span:
            ...
            for strategy in self._parsing_strategies:
                try:
                    ...
                    span.add_event("parsing_strategy_failed", {
            ...
            span.set_attributes({
                ...
                "parser.successful_attempts": len([a for a in parsing_attempts if a.get("success", False)]),
    ...
    async def _parse_json_direct(self, content: str, span: trace.Span) -> Dict[str, Any] | None:
        with tracer.start_as_current_span("parse_json_direct", context=trace.set_span_in_context(span)):
            try:
                return json.loads(content.strip())
            except json.JSONDecodeError:
                return None
    
    async def _parse_json_from_code_block(self, content: str, span: trace.Span) -> Dict[str, Any] | None:
        with tracer.start_as_current_span("parse_json_code_block", context=trace.set_span_in_context(span)):
            patterns = [
                r'```json\s*(.*?)\s*```',
                r'```\s*(.*?)\s*```',
                r'`([^`]+)`'
    ...
    async def _parse_json_from_text(self, content: str, span: trace.Span) -> Dict[str, Any] | None:
        with tracer.start_as_current_span("parse_json_from_text", context=trace.set_span_in_context(span)):
            json_pattern = r'\{.*?\}'
    ...
    async def _parse_key_value_pairs(self, content: str, span: trace.Span) -> Dict[str, Any] | None:
        with tracer.start_as_current_span("parse_key_value_pairs", context=trace.set_span_in_context(span)):
            patterns = [
                r'(\w+):\s*([^\n,]+)',
                r'(\w+)\s*=\s*([^\n,]+)',
                r'"(\w+)":\s*"([^"]+)"'
    ...
    async def _validate_against_schema(self, 
        ...
        with tracer.start_as_current_span("schema_validation", context=trace.set_span_in_context(span)):
            try:
    ...
    async def batch_parse(self, requests: list[ParseRequest]) -> list[ParseResponse]:
        with tracer.start_as_current_span("batch_parsing") as span:
            ...
            for i, request in enumerate(requests):
                ...
                span.add_event(f"batch_item_{i}_completed", {
