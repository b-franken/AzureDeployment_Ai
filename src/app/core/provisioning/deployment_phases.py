...
class DeploymentPhaseManager:
    def __init__(self):
        ...
        self._resource_keywords = [
            'webapp', 'web app', 'app service', 'storage', 'database', 'sql',
            'aks', 'kubernetes', 'container', 'vm', 'virtual machine',
            'keyvault', 'key vault', 'redis', 'cosmos', 'service bus',
            'function', 'logic app', 'api management', 'front door'
        ]
    
    def detect_intent(self, request_text: str, conversation_context: List[Dict[str, str]] = None) -> DeploymentIntent:
        with tracer.start_as_current_span("deployment_phase_detect_intent") as span:
            ...
            if not intent_scores:
                ...
                best_intent = max(intent_scores.keys(), key=lambda i: intent_scores[i]['score'])
                confidence = min(intent_scores[best_intent]['score'] / 5.0, 1.0)
            ...
            if conversation_context:
                ...
                if context_boost:
                    ...
                    reason += f" (boosted by conversation context)"
    ...
    def _analyze_conversation_context(self, conversation_context: List[Dict[str, str]]) -> Optional[DeploymentIntent]:
    ...
    def determine_provisioning_phase(self, context: ProvisionContext) -> ProvisioningPhase:
        with tracer.start_as_current_span("deployment_phase_determine_provisioning") as span:
            current_phase_value = context.current_phase.value if isinstance(context.current_phase, ProvisioningPhase) else str(context.current_phase)
            ...
            span.set_attributes({
                ...
                "phase.reasoning": f"Intent {intent_value} with parsed_resources={len(context.parsed_resources)}, has_plan={bool(context.deployment_plan)}"
            })
            span.set_status(Status(StatusCode.OK))
    ...
    def validate_phase_transition(self, current_phase: ProvisioningPhase, next_phase: ProvisioningPhase) -> bool:
        with tracer.start_as_current_span("deployment_phase_validate_transition") as span:
            valid_transitions = {
                ...
                ProvisioningPhase.PARSING: [ProvisioningPhase.PLANNING, ProvisioningPhase.VALIDATION],
                ProvisioningPhase.PLANNING: [ProvisioningPhase.EXECUTION, ProvisioningPhase.PARSING],
                ProvisioningPhase.EXECUTION: [ProvisioningPhase.FALLBACK, ProvisioningPhase.COMPLETION, ProvisioningPhase.PLANNING],
                ProvisioningPhase.FALLBACK: [ProvisioningPhase.COMPLETION, ProvisioningPhase.EXECUTION],
            ...
            span.set_attributes({
                ...
                "transition.allowed_transitions": [p.value for p in valid_transitions.get(current_phase, [])]
            })
            
            if is_valid:
                span.set_status(Status(StatusCode.OK))
            else:
                span.set_status(Status(StatusCode.ERROR, f"Invalid phase transition from {current_phase.value} to {next_phase.value}"))
    ...
    def get_phase_metrics(self, phase: ProvisioningPhase) -> Dict[str, Any]:
        with tracer.start_as_current_span("deployment_phase_get_metrics") as span:
            metrics = {
                "phase": phase.value,
                "timestamp": tracer.get_current_span().get_span_context().span_id,
                "requirements": self.get_phase_requirements(phase)
            }
            
            span.set_attributes({
                "phase.name": phase.value,
                "phase.required_fields": len(metrics["requirements"].get("required_fields", [])),
                "phase.optional_fields": len(metrics["requirements"].get("optional_fields", []))
            })
            span.set_status(Status(StatusCode.OK))
