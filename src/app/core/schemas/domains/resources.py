...
class AzureResource(BaseSchema, AzureMixin, ValidationMixin):
    ...
    sku: Optional[str] = Field(default=None, description="Resource SKU/pricing tier")
    status: Literal["creating", "running", "stopped", "failed", "deleting"] = Field(default="creating", description="Resource status")
...
class ResourceDependency(BaseSchema):
    ...
    dependency_type: Literal["hard", "soft", "optional"] = Field(default="hard", description="Type of dependency")
    reason: Optional[str] = Field(default=None, description="Reason for the dependency")