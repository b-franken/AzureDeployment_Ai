...
if TYPE_CHECKING:
    ...
    class ChromaDBSettings(Protocol):
    ...
    class ChromaDBModule(Protocol):
        def PersistentClient(self, path: str, settings: Any) -> ChromaDBClient: ...
        def HttpClient(self, host: str, port: int, settings: Any) -> ChromaDBClient: ...

    class ChromaDBCollection(Protocol):
        def upsert(
            self,
            ids: list[str],
...
def _import_chromadb() -> Any:
    """Dynamically import chromadb to handle optional dependency."""
    try:
        return importlib.import_module("chromadb")
    except ImportError as e:
        raise ImportError(
            "chromadb is required but not installed. " "Install it with: pip install chromadb"
        ) from e


def _import_chromadb_settings() -> Any:
    """Dynamically import chromadb.config.Settings to handle optional dependency."""
    try:
...
class ChromaProvider(VectorProvider):
        ...
        with tracer.start_as_current_span("vectordatabase_initialize") as span:
            try:
                ...
                if _chromadb_module is None:
                    _chromadb_module = _import_chromadb()
                if _settings_class is None:
                    _settings_class = _import_chromadb_settings()

                host = self.config.get("host", "localhost")
                port = self.config.get("port", 8000)

                if self.config.get("persistent_path"):
                    self._client = _chromadb_module.PersistentClient(
                        path=self.config["persistent_path"],
                        settings=_settings_class(anonymized_telemetry=False),
                    )
                else:
                    self._client = _chromadb_module.HttpClient(
                        host=host, port=port, settings=_settings_class(anonymized_telemetry=False)
                    )
    ...
    async def create_collection(
        ...
        with tracer.start_as_current_span("vectordatabase_create_collection") as span:
            span.set_attributes(
                {"collection.name": collection_name, "collection.dimension": dimension}
            )
    ...
    async def upsert_vectors(self, collection_name: str, vectors: list[dict[str, Any]]) -> bool:
        with tracer.start_as_current_span("vectordatabase_upsert_vectors") as span:
            ...
            try:
                ...
                span.set_attributes({"upsert.success": True, "upsert.vector_count": len(vectors)})
                span.set_status(Status(StatusCode.OK))
    ...
    async def delete_vectors(self, collection_name: str, vector_ids: list[str]) -> bool:
        with tracer.start_as_current_span("vectordatabase_delete_vectors") as span:
            span.set_attributes(
                {"collection.name": collection_name, "vector_ids.count": len(vector_ids)}
            )
    ...
    async def get_vector_by_id(
        ...
        with tracer.start_as_current_span("vectordatabase_get_vector_by_id") as span:
            span.set_attributes({"collection.name": collection_name, "vector.id": vector_id})

            try:
                collection = self._get_collection(collection_name)

                result = collection.get(
                    ids=[vector_id], include=["documents", "metadatas", "embeddings"]
                )
    ...
    def _get_collection(self, collection_name: str, auto_create: bool = True) -> Any:
                ...
                if auto_create:
                    try:
                        ...
                        raise ValueError(
                            ...
                            f"and auto-creation failed: {str(create_error)}"
                        ) from e
                else:
    ...
    async def get_stats(self, collection_name: str) -> dict[str, Any]:
        with tracer.start_as_current_span("vectordatabase_get_stats") as span:
            ...
            try:
                ...
                span.set_attributes({"stats.vector_count": count, "stats.status": stats["status"]})
                span.set_status(Status(StatusCode.OK))
    ...
    async def similarity_search(
        ...
        with tracer.start_as_current_span("vectordatabase_similarity_search") as span:
            span.set_attributes({"search.embedding_dim": len(embedding), "search.limit": limit})
    ...
    async def cleanup_old_vectors(self, max_age: int) -> int:
                ...
                for collection_name in list(self._collections.keys()):
                    ...
                    if all_vectors["ids"]:
                        for i, vector_id in enumerate(all_vectors["ids"]):
                            metadata = (
                                all_vectors["metadatas"][i] if all_vectors["metadatas"] else {}
                            )
